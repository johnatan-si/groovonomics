...
Types allow the compiler to find type errors sooner and potentially improve the readability and maintainability of a program.
On the other hand, "untyped" code may be easier to change and require less work.
...
There is no correlation between the size and age of projects and how they are typed.
...
Dynamically typed languages they tend to allow programmers to code faster \cite{types_and_programming_languages} and adapt to frequently changing requirements more easily \cite{gradual_typing}.
...
Through the analysis of a massive dataset with almost seven thousand Groovy projects, we were able to identify when programmers preferred to typed or not their declarations.
Our results bring more visibility about what is the point of view of programmers about the use of types.
This analysis complements, with a different point of view, existing studies based on controlled experiments.
...
We propose the following research questions about the usage of types by Groovy programmers:
...
\begin{itemize}
	...
	\item \textbf{Question Q3: Does the size, age or level of activity have any influence on the usage of types?} We hypothesize that as these metrics grow, there is an increased concern about keeping code more maintainable. This can lead programmers to use types more often as a means to improve code readability.
...
This is explained by the fact that, different from Java, in Groovy, the default visibilities for fields and methods are private and public respectively.
...
This sections presents the data obtained form our analysis and its statistical treatment.
We show how the usage of types varies according to kind and visibility of declarations, nature of code, programmers' background, project maturity and frequency of changes.
The interpretation of these results is left to Section \ref{discussion}, which answers the proposed research questions based on this data and raises new research questions.
...
\begin{description}
\item[H0] The data of the two samples are drawn from the same population, i.e., there is no difference in how programmers type different kinds of declarations
...
This value might seem too small at first, which would require the difference between two samples to be unnecessarily high in order to considered significant.
...
\hline
\hline
Field & Contructor Parameter & 0      & (-0.40, -0.34) \\ \hline
Local Variable & Contructor Parameter & 0      & (-0.54, -0.48) \\ \hline
Method Parameter & Contructor Parameter & 0      & (-0.16, -0.10) \\ \hline
Method Return & Contructor Parameter & 0      & (-0.15, -0.09) \\ \hline
Local Variable & Field & 0      & (-0.15, -0.11) \\ \hline
Method Parameter & Field & 0      & ( 0.22,  0.26) \\ \hline
Method Return & Field & 0      & ( 0.22,  0.26) \\ \hline
Method Parameter& Local Variable & 0      & ( 0.35,  0.40) \\ \hline
Method Return & Local Variable & 0      & ( 0.36,  0.40) \\ \hline
Method Return & Method Parameter& 0.95 & (-0.01,  0.02) \\ \hline
\end{tabular}
...
This means that there is no significant difference in how programmers use types in these two kinds of declaration.
...
Public & Private & 0 & ( 0.24,  0.27) \\ \hline
Public & Protected & 0 & (-0.26, -0.22) \\ \hline
\end{tabular}
...
Instead, we are comparing declarations in test classes to declarations of the same kind in main classes.
Results for the U test are shown in Table \ref{tab:test_utest_type}.
We do not show results by visibility since this is usually not a concern when writing test classes.
...
We analyze three metrics: age, number of lines of code and number of commits.
...
All in all, we can not say that there is a correlation between the metrics and the usage of types in Groovy projects.
\begin{table}[h!]
...
The lack of correlation between usage of types and these metrics does not necessarily imply that they no influence on the usage of types.
...
Hence, we can not conclude that programmers type their declarations differently in mature projects, at least, when considering our definition of maturity.
...
This is supported by Section \ref{dataset}, which shows that the great majority of field declarations is private.
