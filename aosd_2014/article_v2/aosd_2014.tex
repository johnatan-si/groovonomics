...
\begin{itemize}
	\item \textbf{Question Q1: Do programmers use types to implicitly document the interface of their modules?} When programmers use types in the interface of a module, they are specifying how other modules should interact with it. This potentially improves the readability and maintainability of their programs. While, the usage of types in the definition of modules has been previously addressed by experimental studies, we want to find if programmers in fact consider this strategy when writing their programs.
...
% Q1
\subsection*{Q1: Do programmers use types to implicitly document the interface of their modules?\label{discussion-q1}}
The results presented in this paper show that declarations that define the interface of a module are typed much more frequently than others.
We believe that the main motivation for this is the implicit documentation provided by the use of types.
Type declarations in module interfaces provide useful hints about the behavior of these modules \cite{Curtis1987}, potentially improving the readability and maintainability of programs \cite{Iso2004}.

The results presented in Section \ref{sec:results-type} reveal that methods and constructors, which can be part of a module interface, are the most typed declarations.
In particular, constructors, which are extremely important members of a module definition, have their parameters always typed typed in most projects.
...
This is supported by the fact that the great majority of field declarations are private, as shown in Section \ref{dataset}.
% TODO characterize declaration types by visibility
Local variables, on the other hand, which are not exposed as part of a module interface, are frequently untyped.

% Users of well typed modules may learn how to use them faster and make less mistakes when interacting with them.
%  \cite{Meyer88, Meijer04, Wadler04, Plosch97, Flanagan2006, Furr09}

Section \ref{sec:results-visibility} shows that private declarations, which are hidden from other modules, are typed much less than public and protected declarations.
...
In addition, protected methods are usually designed to be overwritten by the subclasses of a module.
Apparently programmers understand that the contract defined by protected declarations are more delicate than others and that hey should be well document with the use of types.

It is very important to say that  documentation might not be the only reason why programmers type declarations in modules interfaces.
For example, a programmer might type a declaration of a method parameter so he can get code completion to work for that parameter inside the method.
Another possibility is that, even though Groovy is actually a dynamically typed language, programmers might type their declarations thinking that the compiler will check for type errors.

% Q2
\subsection*{Q2: Are untyped declarations preferred when neither readability nor stability is a concern?\label{discussion-q2}}
Programmers type their declarations less frequently when neither readability nor stability is a concern.
This is what we found analyzing the usage of types in local variables, private declarations, test classes and script files.
We believe that in these scenarios, using types is not as advantageous and programmers seem to prefer the simplicity and flexibility of untyped declarations.

As discussed in the previous section, local variables and private declarations, which are not part a module interface, are typed less frequently.
Both stability and readability are not a concern for these declarations.
Since they are not exposed to other modules, changing them causes less impacts in the program.
Also, it is easier to read code with untyped declarations in these scenarios.
The value of a local variable is defined very close to its declaration, usually in the same statement, making it easier to infer its type.
A reader might have more work to do the same for private elements, but this is potentially easier than in the case of public or protected declarations.
