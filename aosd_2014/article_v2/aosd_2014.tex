...
\begin{abstract}
The recent popularization of dynamically typed languages, such as Ruby and JavaScript, has brought more attention to the discussion about the impacts of typing strategies over development.
...
An analysis of the source code of 6638 projects written in Groovy, a programming language which features optional typing, shows in which scenarios programmers prefer typing or not their declarations. 
Results show that types are popular in the definition of module interfaces, but are less used in scripts, test classes and frequently changed code.
There is no correlation between the size and age of projects and how their constructs are typed.
Finally, we also found evidence that the background of programmers has influence on how they use types.
\end{abstract}
...
A programming language's type system determines when the type of a statement is defined \cite{types_and_programming_languages}. 
...
Our results give more visibility about what is the point of view of programmers about the use of types.
...
However, an exception is thrown when the method \emph{toUpperCase} is invoked since the \emph{Integer} class does not have such method.
...
Its goal is to find in which contexts, Groovy programmers type or not their declarations.
In this section, we present five research questions that guide our analysis, the data collection and analysis procedures as well as the characterization of the studied dataset.
...
\begin{itemize}
	...
	\item \textbf{Question Q4: Does the size, age or level of activity have any influence on the usage of types?} We hypothesize that as these metrics grow, there is an increased concern about keeping code more maintainable. This can lead programmers to use types more often as a means to improve code readability.

	\item \textbf{Question Q5: In frequently changed code, do developers prefer typed or untyped declarations?} It makes sense to assume that developers try to increase the maintainability of frequently changed code. One way to achieve that is improving the readability of such code with the use of types. On the other hand, the flexibility of untyped declarations is capable of increasing the changeability of those files. We want to understand which one of these strategies is actually preferred by Groovy developers in the end.
...
More details about these declarations are shown in Tables \ref{tab:number_of_declarations} and \ref{tab:number_of_declarations_by_visibility}.
...
While 96\% of the projects were developed by small groups of 3 people or less, there were projects with up to 58 people involved.
...
The downside of this approach is that we can not analyze Groovy code in conjunction with its dependencies. 
...
This sections presents the data obtained from our analysis and its statistical treatment.
...
We initially considered not to include these projects in the rest of our analysis since they could not represent well the entire population of Groovy projects.
...
% TYPE
\subsection{Declaration Kind\label{sec:results-type}}
...
For instance, there are only 1670 out of 6638 projects with declarations of constructor parameters.
...
We first considered applying repeated \emph{t-tests} or the \emph{Mann-Whitney U-tests} to compare every two kinds of declaration, i.e, fields vs local variables, fields vs method returns, etc.
However, applying repeated tests over the same sample increases the probability of getting Type-I errors (rejecting the null hypothesis when it actually should should be accepted).
...
The ANOVA test reported \emph{p} value equal to 0 for these treatments, allowing us to strongly reject the null hypothesis.
...
Finally, results of the Tukey HSD Test are reported in Table \ref{tab:all_utest_visibility}.
...
These results suggest a positive answer for Question Q1, which hypothesizes that declarations that are part of a module definition are typed more frequently.
...
The convention adopted by popular build tools among Groovy programmers, such as Gradle and Maven, assumes that test classes and main classes are in the \emph{src/test/groovy} and \emph{src/main/groovy} directories respectively.
...
In order to compare the usage of types in test and functional classes, we use a slightly different approach.
...
Multiple values of \emph{p} are calculated by this test, one corresponding to the comparison of treatments according to one of the factors.
We report the \emph{p} value corresponding to the factor representing the kind of class.
We also apply the Tukey HSD test, which results are displayed in Table \ref{tab:test_utest_type}.
...
For example, the third row of Table \ref{tab:test_utest_type} shows that there is a significant difference in how programmers type local variables in functional and test classes.
The difference between the relative usage of types in functional classes and test classes falls in the $(0.05, 0.11)$ interval.

The ANOVA test reported once again a \emph{p} value equal to 0, implying that the usage of type is different in test and functional classes.
...
Local Variable &  0          & ( 0.047,  0.11) \\ \hline
Method Parameter &  0          & ( 0.36,  0.44) \\ \hline
Method Return &  0          & (-0.31, -0.26) \\ \hline
\end{tabular}
\caption{Tukey Honest Significant Differences Test results for the comparison between the usage of types by functional and test classes}
...
Figure \ref{fig:script_boxplot_type} shows the distribution of the relative usage of types in class files and script files.
...
Along with the results of the analysis of test classes, these results show that the answer for Question Q2 is positive.
...
The \emph{p} value reported by the ANOVA test is equal to 0, showing that there is a significant difference in how programmers with different backgrounds type their declarations.
...
There are other two cases which do not present significant differences, method returns and public declarations, both in the comparisons between programmers of the \emph{Dynamic} and \emph{Static} groups.
...
All in all, these values don not seem to suggest any direct relationship between these metrics and the usage of types.
...
This result, along with the very low correlation coefficients reported in Table \ref{tab:all_correlation_maturity}, imply that the answer for Question Q4 is negative.
...
In projects where programmers prefer the usage of types in frequently changed files, this coefficient is positive, and negative when untyped declarations are preferred.
...
The analysis of the usage of types by kind and visibility of declarations, presented in Sections \ref{sec:results-type} and \ref{sec:results-visibility}, shows evidence that the answer for Q1 is affirmative.
...
Constructor parameters and protected declarations are always the most typed elements.
In addition, the analysis of different contexts, i.e, functional and test classes, mature and non-mature projects and programmers' backgrounds, shows no significant difference in the usage of types.
This reveals that the presence of these particular declarations are considered by programmers as more important factors in their choice whether to type a declaration than these contexts.
...
In addition, they might be the first element of a class which programmers interact with when dealing with a new module.
Protected declarations are often used as a means to delegate the implementation of a method to subclasses, which requires an well defined contract so the superclass can work properly.
...
We can also speculate that declarations that are not part of a module definition, local variables or those with private visibility, require less documentation and thus are typed less often.
...
We can think of at least other two reasons.
...
As found in the recent work of Hanenberg et al \cite{Hanenberg13}, untyped code potentially has positive impacts on the development time of easier tasks.
...
In general, those programmers who have only statically typed languages in their portfolio type more often than all others.
...
Such enterprise projects are probably hosted privately on GitHub or in private servers, hence unavailable to us.
...
One of the goals of this paper is to complement theses results by providing a a different point of view to their analyses. 
...
In experiments conducted by Daly et al. \cite{ruby_vs_druby}, the authors compare the performance of two groups working on small development tasks.
...
We hope that the questions raised in our discussion can inspire other researchers to analyze our findings in more details.
...
Another work in progress is a framework for the static source code analysis of large numbers of repositories called Elastic Repository Analysis - ERA\footnote{http://github.com/carlosgsouza/ERA}.
This framework is based on the artifacts we built to analyze the Groovy projects in this study.
ERA will help researchers to retrieve large numbers of projects from GitHub and process these projects quickly on the cloud using Amazon Web Services.
