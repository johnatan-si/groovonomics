...
Groovy offers a native syntax for lists(lines 3, 6 and 14) and operator overloading (line 6). 
...
This closure is passed down to the $findAll$ method, which will apply this closure to every element of the list to decide if that element should be returned or not.
Closures are one of the most importante features of Groovy compared to Java.
...
In this section we describe the dataset (subsection \ref{dataset}) and the methodology (subsection \ref{analyzer}) adopted in this study.

\subsection{Research Questoins\label{questions}}

\begin{itemize}
	\item \textbf{Question Q1:} Do programmers use types to implicitly document their code? More specifically, is it possible to observe a higher usage of typing in statements that require more documentation? We consider two characteristics of statements to determine if they need to be more documented than others, the number of modules to which a statement is visible and the complexity of the contract defined by the statement. The higher the number of modules that can access a statement, the more it need to be documented since bad code can break the entire project and other people might need to see that statement. Same for contract complexity. If the contract is not defined correctly, things will explode.
	
	\item \textbf{Question Q2:} Are untyped declarations considered more often when maintinability or readability aren't important factors? If we are able to observe a higher usage of untyped declarations in this scenario, it means that programmers only use typing when they need maintinability and readabaility and in other cases, they will favor the objectivity of untyped declarations.
	
	\item \textbf{Question Q3:} Is the previous experience of a programmer a relevant factor in his or her decision for typing or not typing a statement? I mean, previous experience with typing systems. If yes, we can consider that programmers learn how to deal with type systems and can overcome advantages and disadvantages when they have to. This also imposes a threaten to the above two questions since a programmer might not use types because of the factors above, but because he likes it more.
\end{itemize}

\subsection{Study Design\label{design}}
We will try to answer the questions proposed in section ?? by conducting a case study.
We will use a large dataset of Groovy projects to analyze this data.
Groovy allows programmers to combine typed and untyped statements flexibly in their code, which makes it possible to answer the questions, where programers use typed or untyped statements. We can cross reference that to the questions above and see if there is any correspondence between the use of types and the hyposthesis above.
...
\subsection{Analysis\label{design}}
