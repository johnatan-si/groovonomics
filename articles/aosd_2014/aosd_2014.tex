...
When maintainability or readability are not a concern, Groovy programmers tend to use more untyped declarations.
The flexibility and objectivity of dynamic typing becomes more visible to programmers in these scenarios.
The analysis of the results by declaration type, visibility and nature of the code support this understanding and complement the analysis of Question Q1 discussed previously.
...
It can be assumed that typing local variables doesn't aid in the readability of the code as much as in other types of declarations.
...
This makes changeability in local variables a smaller concern and make the flexibility of untyped declarations more apparent.
For example, one can easily change the type of a local variable without having to worry about it use anywhere else.

A similar analysis can be made for private declarations.
They are only visible inside the class they were defined in.
In this scenario, readability and changeability aren't big concerns, which allow programmers to benefit form the flexibility and objectivity of dynamic typing more often.

Test classes are typed less often than the main classes of projects.
Figure \ref{tab:tests} shows that, while declarations made inside the main classes of projects are typed in 68\% of the cases, only 56\% of the test classes are typed.
These classes usually have simple relationships with the rest of the project.
They don't have any clients and depend only on the classes under test.
In this scenario, readability or changeability are usually not a concern.

A counter example to the analysis made so far is the usage of types in scripts.
It can also be said that readability and changeability are not big concerns in scripts.
These are pieces of code usually written to solve punctual problems.
They don't establish complex relationships with other modules aren't often reused.
Still, as shown in Table \ref{tab:scripts}, there is no significant difference between the overall usage of types in scripts and classes.
