...
The most evident benefit of using types for a Groovy programmer is documenting their code.
Since Groovy has a dynamic type system (except when type checking is explicitly activated), the compiler isn't capable of checking for errors.

To answer this question, we will analyze how programmers type the interfaces of their modules.
...
Programmers can benefit from well typed modules as these will tell him how to use them.




This points that contracts require documentation.
If contracts require documentation and developers use types in contracts, it means they are using it to document the code.


In our analysis, we were able to find evidence that programmers often consider types as a means to document their code.

We were able to find evidence that types are considered as documentation
...
Figure 5 shows that while local variables are very untyped other types of declarations are typed a lot
Specially constructors which define very special contracts

The analysis provided by visibiliy confirms this idea
private elements are always less typed than others
these don't specify an interface
only the own class can access that

Protected are the most typed
they specify an intimate contract with a subclass
that subclass has access to the inners of a superclass


Inheritance coupling is high \cite{Eder94}
