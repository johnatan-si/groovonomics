...
When readability or changeability are not a concern, the advantages of typing become less apparent and  programmers prefer the flexibility and objectivity of untyped declarations.
This is what we found when we analyzed how typing is used in local variables, private statements and test classes.
These are scenarios that we consider to require less readability and changeability than others.
These results complement Question Q1 discussed previously.
...
Changeability in this scenarios is not as critical making he flexibility of untyped declarations more apparent.
For example, one can easily change the type of a local variable without having to worry about its use anywhere else.

A similar analysis can be made for private statements.
These are only visible inside the class they were defined in.
As shown in Figures \ref{fig:method_return_visibility} and \ref{fig:method_parameter_visibility},\ref{fig:constructor_parameter_visibility} and \ref{fig:field_visibility}, untyped declarations are much more common in private statements that in their public or private counterparts.

Another scenario with smaller need for readability and changeability are test classes.
...
A counter example to this analysis made so far is the usage of types in scripts.
...
They don't establish complex relationships with other modules and aren't reused very often.
